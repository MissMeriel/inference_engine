...
public class BayesianEvent<T> extends TypedEvent{
   ...
   public String generate_bayesian_probability(HashMap<String, HashMap<String, Double[]>> cumulative_probabilities){
      ...
      for(String key1 : keys1){
         ...
         for(String key2 : keys2){
            ...
            if(debug) out.format("pA = get_prior(%s, %s)%n", var_name, val.toString());
   ...
   public Double[] get_cumulative_probability(String voi_name, String val, HashMap<String, HashMap<String, Double[]>> cumulative_probabilities){
      ...
      try{
         ...
         /*if(debug) {
            out.format("get_cumulative_probability: d=%f%n",event_val_count);
            out.format("get_cumulative_probability: d==Double.MAX_VALUE: %s%n",(event_val_count==Double.MAX_VALUE));
         }
         if(event_val_count == Double.MAX_VALUE){
            if(debug) out.format("get_cumulative_probability: %s:%s not found in existing events for %s:%s%n",voi_name, val, this.var_name, this.val);
            throw new NullPointerException(String.format("get_cumulative_probability: %s:%s not found in existing events for %s:%s%n",voi_name, val, this.var_name, this.val));
         }*/
      }
      return d_arr;
   }
